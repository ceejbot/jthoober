...
Rule.prototype.test = function(event)
{
    ...
    isRepo = (this.pattern.test(event.payload.repository.name));

    if (this.branchPattern){
        isBranch = this.branchPattern.test(event.payload.ref.replace('ref/heads/', ''))
    }
    else {
        isBranch = true
    }
...
function emitComplete(self, exitCode){
    setImmediate(function ensureLogStreamsAreFlushed(){
        self.emit('complete', exitCode || 0, self.bufferedError);
