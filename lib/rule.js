...
function getBranchName(event){
    var branchName
    if (event.payload.ref) branchName = event.payload.ref.replace('ref/heads/', '')
    return branchName
}

function emitComplete(self, exitCode, event){
    setImmediate(function ensureLogStreamsAreFlushed(){
        self.emit('complete', exitCode || 0, self.bufferedError, getRepoName(event), getBranchName(event));
...
Rule.prototype.test = function(event)
{
    ...
    isRepo = (this.pattern.test(getRepoName(event)));

    if (this.branchPattern){
        isBranch = this.branchPattern.test(getBranchName(event))
    }
    else {
        isBranch = true
    }
