...
function emitComplete(self, exitCode, event)
{
    setImmediate(function ensureLogStreamsAreFlushed()
    {
        self.emit('complete',
                exitCode || 0,
                self.bufferedError,
                self.getRepoName(event),
                self.getBranchName(event));
...
Rule.prototype.exec = function(event)
{
    ...
    self.logger.info('starting execution; cmd=' + cmd + '; hostname=' + os.hostname());
    ...
    ps.stdout.on('data', function (data)
    {
        ...
        if (/error|fail|fatal/i.test(data)) self.logger.error(data);
