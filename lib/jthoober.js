	...
	this.rules.forEach(function(rule)
	{
		if (rule.event === event.event || rule.event === '*')
		{
			if (rule.test(event))
			{
				...
				rule.once('complete', function onRuleComplete(exitCode, errOutput, repo, branch)
				{
					...
					if (exitCode)
						...
						slacker.report('Execution complete. ' + repo + ': ' + (branch ? branch + ': ' : '') + rule.event + '; hostname=' + os.hostname(), rule.slack);
				});
			}
			else
				self.logger.info(rule.getBranchName(event) + ' does not match; ignoring');
		}
	});
};
